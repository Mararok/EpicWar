...
/**
 * Simple command arguments wrapper. Supports subcommands in args, type checking
 * methods and required args amount check
 */
...
public class CommandArguments {
  ...
  public boolean isFloat(int index) {
    return isExists(index) && StringUtil.isFloat(get(index));
  }

  public boolean isInteger(int index) {
    return isExists(index) && StringUtil.isInteger(get(index));
  }
  ...
  public CommandArguments getArgumentsForChild(PluginChildCommand command) {
    if (getAmount() >= command.getDepthLevel()) {
      return new CommandArguments(Arrays.copyOfRange(rawArguments,
          command.getDepthLevel(), getAmount()));
    }
