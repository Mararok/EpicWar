...
public abstract class PluginParentCommand extends PluginChildCommand {
  ...
  public PluginParentCommand(EpicWarPlugin plugin, String name,
  ...
  private boolean execSubCommand(CommandSender sender,
      CommandArguments arguments) {
    String subCommandName = arguments.asString(0);
    PluginChildCommand childCommand = getCommandByName(subCommandName);
    if (childCommand != null) {
      return childCommand.onCommand(sender,
          arguments.getSubArguments(childCommand.getDepthLevel()));
    }
    sender.sendMessage("Command: " + subCommandName
        + " not exists or not implemented yet");
    return false;
  }
  ...
  public void sendDescription(CommandSender sender) {
    ...
    subCommandsInfo[0] = ChatColor.YELLOW + "" + ChatColor.BOLD
        + getDescription();
    int i = 1;
    for (PluginCommand sub : SubCommandsList) {
      subCommandsInfo[i] = sub.getDisplayName() + ChatColor.RESET + " - "
          + sub.getDescription();
      ++i;
    }
