...
/**
 * EpicWar The MIT License Copyright (C) 2015 Mararok <mararok@gmail.com>
 */
...
public class Airdrop implements MetadataValue {
  ...
  private void create() {
    ...
    if (loc == null) {
      ...
      // Summoner.getPlayers().getWar().getairdropConfig();
      Summoner.getWorld().spawnFallingBlock(loc, Material.ANVIL, (byte) 0)
          .setMetadata("airdrop", this);
      // Summoner.subPoints()
    }
  }
  ...
  private Location findLocation() {
    ...
    int height = UMath.getRandomIntFromRange(Generator, config.heightMin,
        config.heightMax);
    int radius = UMath.getRandomIntFromRange(Generator, config.radiusMin,
        config.radiusMax);
  ...
  public void onFalled(Block block) {
    ...
    Summoner
        ...
        .getPlugin()
        .logInfo(
            "airdrop falled at: " + loc.getBlockX() + "," + loc.getBlockY()
                + "," + loc.getBlockZ());
    Summoner.sendMessage("airdrop falled, distance "
        + loc.distance(Summoner.getBPlayer().getLocation()));
    AirdropConfig config = Summoner.getWar().getAirdropConfig();
    int slots = UMath.getRandomIntFromRange(Generator, config.slotsMin,
        config.slotsMax);
    Summoner.getWorld().getBlockAt(loc).setType(Material.CHEST);
    Chest chest = (Chest) Summoner.getWorld().getBlockAt(loc).getState();
    ItemStack stack;
    for (int i = 0; i < slots; ++i) {
      stack = config.ItemsDrop.get(Generator.nextInt(config.ItemsDrop.size()))
          .getRandom(Generator);
      if (stack != null)
