...
import com.gmail.mararok.bukkit.util.ChanceGenerator;
...
public class AirdropItem {
  ...
  public AirdropItem(Material itemType, int minAmount, int maxAmount, int chance) {
    ItemType = itemType;
    MinAmount = (minAmount < itemType.getMaxStackSize()) ? minAmount : itemType
        .getMaxStackSize() - 1;
    MaxAmount = (maxAmount >= itemType.getMaxStackSize()) ? maxAmount
  ...
  public ItemStack getRandom(Random generator) {
    if (ChanceGenerator.calcChance(Chance)) {
      ItemStack stack = new ItemStack(ItemType, generator.nextInt(MaxAmount
          - MinAmount)
          + MinAmount);
      return stack;
    }
