...
public class AirdropConfig {
	...
	public static AirdropConfig createFromWarConfig(EpicWarPlugin plugin, FileConfiguration config) {
		AirdropConfig adConfig = new AirdropConfig();
		adConfig.enable = config.getBoolean("airdrops.enable");
		if (adConfig.enable) {
			Map<String,Object> items = config.getConfigurationSection("airdrops.items.list").getValues(false);
			for (String key : items.keySet()) {
				plugin.logInfo("item: "+key);
				adConfig.ItemsDrop.add(new AirdropItem(
					Material.getMaterial(key.toUpperCase()),
					config.getInt("airdrop.items.list."+key+".amount.min"),
					config.getInt("airdrop.items.list."+key+".amount.max"),config.getInt("airdrop.items.list."+key+".chance")));
			}
