...
public class Occupiers {
  ...
  private List<WarPlayerImpl> players = new LinkedList<WarPlayerImpl>();
  
  private ControlAreaImpl controlArea;
  
  public Occupiers(ControlAreaImpl controlArea) {
    this.controlArea = controlArea;
  }
  
  public void addOccupant(WarPlayerImpl occupant) {
  ...
  private void addOwnerOccupant(WarPlayerImpl occupant) {
  ...
  private void addEnemyOccupant(WarPlayerImpl occupant) {
  ...
  public void removeOccupant(WarPlayerImpl occupant) {
    if (isOwnerOccupant(occupant)) {
  ...
  public boolean isOwnerOccupant(WarPlayerImpl occupant) {
    return occupant.getFaction() == controlArea.getOwner();
  }
  
  private void removeFriendOccupantPlayer(WarPlayerImpl warPlayer) {
    --ownerOccupiersAmount;
  }

  private void removeEnemyOccupantPlayer(WarPlayerImpl warPlayer) {
    --enemyOccupiersAmount;
  }
  ...
  private void capture() {
    ...
    String newOwnerName = controlPoints.getWar().getFactionManager()
  ...
  private OccupantInfo findNewFactionOccupant() {
    short[] factionOccupants = new short[controlArea.getWar().getFactionManager()
        .size() + 1];
