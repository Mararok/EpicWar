...
public class FactionManager extends DataSetManager<Faction> implements Disposable {
	public static final int NEUTRAL_ID = 0;
	private HashMap<ChatColor,Faction> FactionsColors;
	
	public FactionManager(War war) throws Exception {
		super(war);
	...
	public void load() throws Exception {
		...
		PreparedStatement factionsInfoStatement = DBConnection.get().prepareQuery(
		...
		Faction faction;
		while (results.next()) {
			faction = new Faction(FactionData.fromDBResults(results),this);
	...
	public void create(FactionData info) {
		try {
			PreparedStatement st = DBConnection.get().prepareQuery(
