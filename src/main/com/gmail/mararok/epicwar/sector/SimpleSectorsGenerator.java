...
public class SimpleSectorsGenerator implements SectorsGenerator {
  ...
  public void setConfig(ConfigurationSection config)
    ...
    if (SectorSize < 64) {
      throw new SectorsGeneratorConfigException("size", "you can't set "
          + SectorSize + " value, min is 64");
    }

    if (WidthInSectors < 2) {
      throw new SectorsGeneratorConfigException("width", "you can't set  "
          + WidthInSectors + " value, min is 2");
    }

    if (HeightInSectors < 2) {
      throw new SectorsGeneratorConfigException("height", "you can't  "
          + HeightInSectors + " value, min is 2");
    }

    if (Names.size() == 0) {
      throw new SectorsGeneratorConfigException("names",
          "you don't define names for sectors");
    }

    if (Names.size() != WidthInSectors * HeightInSectors) {
      throw new SectorsGeneratorConfigException("names",
          "you don't define all names for sectors " + Names.toString());
    }
  }
  ...
  public List<SectorInfo> generate(Location creatorLocation) {
    ...
    List<SectorInfo> sectors = new ArrayList<SectorInfo>(WidthInSectors
        * HeightInSectors);
